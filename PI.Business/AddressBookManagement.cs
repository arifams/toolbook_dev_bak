        ...
        public PagedList GetAllAddresses(string type, string userId, string searchtext, int page = 1, int pageSize = 25)
        {
            ...
            foreach (var item in content)
            {
                pagedRecord.Content.Add(new AddressBookDto
                {
                    ...
                    FullAddress = item.StreetAddress1 + 
                                  (string.IsNullOrWhiteSpace(item.StreetAddress2) ? "" : ", " + item.StreetAddress2) ,  
                ...
                if (item.Count != 0)
                {
                    ...
                    if (detailsarray.Length != 0)
                    {
                        addressList.Add(new AddressBook()
                        {
                            ...
                            FirstName = detailsarray[0].ToString(),
                            LastName = detailsarray[1].ToString(),
                            CompanyName = detailsarray[2].ToString(),
                            ZipCode = detailsarray[3].ToString(),
                            //Number = detailsarray[5].ToString(),
                            StreetAddress1 = detailsarray[4].ToString(),
                            StreetAddress2 = detailsarray[5].ToString(),
                            City = detailsarray[6].ToString(),
                            State = detailsarray[7].ToString(),
                            EmailAddress = detailsarray[8].ToString(),
                            PhoneNumber = detailsarray[9].ToString(),
                            Country = detailsarray[10].ToString(),
                            AccountNumber = detailsarray.Length > 11 ? detailsarray[11].ToString() : string.Empty,
                            CreatedBy = userId,
